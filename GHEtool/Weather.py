...
INSTALLATION_TILT = 35
INSTALLATION_AZIMUTH = 180
...
class Weather:
    ...
    __slots__ = "temperature", "DNI", "DHI", "ETradiation", "GHI", "DNIL", "GHIL",\
                "location", "solarIrradianceVar", "solpos", "temperatureExternalWall", "wind_speed"
    ...
    def __init__(self, weatherfile: str, coordinates: tuple = (50.79690, 4.35810), height: float = 46) -> None:
        ...
        if weatherfile.split(".")[-1] == "epw":
            TMY: pd.DataFrame = pd.read_csv(weatherfile, sep=",", header=None, skiprows=8)
            # drop first 6 columns
            TMY.drop(columns=TMY.columns[:5], axis=1, inplace=True)
        else:
            TMY: pd.DataFrame = pd.read_csv(weatherfile, sep="	", header=None)
        ...
        self.wind_speed = np.array(TMY.iloc[:, 16])
        ...
        self.temperatureExternalWall = self.temperature + 0.5/25 * np.sum(self.solar_irradiance, axis=0) / 4
    ...
    def calculate_solar(self) -> np.array:

        # set times
        times = pd.date_range('2018-01-01 00:00:00', '2018-12-31 23:59:00',
                              inclusive='left', freq='H', tz=self.location.tz)
        solpos = self.location.get_solarposition(times)
        ...
        temp = pvlib.irradiance.get_total_irradiance(
            INSTALLATION_TILT, INSTALLATION_AZIMUTH, zenith, azimuth, self.DNI, self.GHI, self.DHI,
            model="haydavies", dni_extra=self.ETradiation, albedo=0)
        Gt = temp["poa_global"]
    ...
    @property
    def solar_irradiance(self) -> np.array:
        """
